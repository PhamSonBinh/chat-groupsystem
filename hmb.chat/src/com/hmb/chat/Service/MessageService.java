...
public class MessageService {
	...
	public boolean deleteMessage(int senderId, int receiverId, String content) {
		Message tempMessage = storage.getMessages().getFirst(message -> message.getSenderId() == senderId
				&& message.getReceiverId() == receiverId && message.getMessageContent().equalsIgnoreCase(content));
		if (tempMessage != null) {
	...
	public List<Message> getMessageListBetween(int senderId, int receiverId) {
		return storage.getMessages()
				.get(message -> message.getSenderId() == senderId && message.getReceiverId() == receiverId);
	}

	public List<Message> getTopLatestMessage(int senderId, int receiverId, int numberOfLatestMessages,
			int numberOfExcludedMessages) {
		...
		for (int index = messageListSize - numberOfExcludedMessages - numberOfLatestMessages; index < messageListSize
				- numberOfExcludedMessages; index++) {
